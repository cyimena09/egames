...
$newPlayer = $player + 1; // les joueurs sont encodés 1 à 1, 'newPlayer' correspond au joueur suivant
...
// et qu'une fois l'équipe et le jeu encodés, l'inscription sera terminée.
    ...
    if (empty($game)) {
        ...
        if (count($_SESSION['players']) == $maxPlayer) {
            foreach ($players as $player) {
                $playerId = createPlayer($player, $teamId); // On insert le joueur et l'id de son équipe dans la colonne FK_Team (clé étrangère).
            ...
            // une fois l'inscription cloturée on détruit la session et on redirige vers une page de succès
            session_destroy();
...
// --------------------------------------------------------------------------------------------------
/* Si les données sont correctes on peut passer à l'insertion du joueur suivant */
// --------------------------------------------------------------------------------------------------
